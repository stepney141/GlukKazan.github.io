...
function AllCutNode(ply, depth, beta, allowNull) {
    ...
    if (!g_inCheck &&
        ...
        if (ply > 1 &&
            ...
            (g_pieceCount[pieceAlfil | g_toMove] != 0 ||
             g_pieceCount[pieceAsb | g_toMove] != 0 ||
             g_pieceCount[pieceRokh | g_toMove] != 0 ||
             g_pieceCount[pieceFers | g_toMove] != 0)) {
...
function ResetGame() {
    ...
    pieceSquareAdj[pieceSarbaz]   = MakeTable(pawnAdj);
    pieceSquareAdj[pieceAsb] = MakeTable(knightAdj);
    pieceSquareAdj[pieceAlfil] = MakeTable(bishopAdj);
    pieceSquareAdj[pieceRokh]   = MakeTable(rookAdj);
    pieceSquareAdj[pieceFers]  = MakeTable(bishopAdj);
    pieceSquareAdj[pieceShah]   = MakeTable(kingAdj);

    var pieceDeltas = [[], [], g_knightDeltas, g_bishopDeltas, g_rookDeltas, g_queenDeltas, g_kingDeltas];
    ...
    for (var row = 0; row < 0x80; row += 0x10) 
        for (var col = 0; col < 0x8; col++) {
            ...
            g_vectorDelta[index].pieceMask[colorWhite >> 3] |= (1 << pieceSarbaz);
    ...
    InitializeEval();
    InitializeFromFen("rnbkqbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBKQBNR w");
}
...
function InitializeFromFen(fen) {
    ...
    g_move50 = 0;
    g_inCheck = IsSquareAttackable(g_pieceList[(g_toMove | pieceShah) << 4], them);

    // Check for king capture (invalid FEN)
    if (IsSquareAttackable(g_pieceList[(them | pieceShah) << 4], g_toMove)) {
...
function MakeMove(move){
    ...
    if (flags & moveflagPromotion) {
        ...
        g_baseEval -= materialTable[pieceSarbaz];
    ...
    if ((piece & 0x7) == pieceShah || g_inCheck) {
        if (IsSquareAttackable(g_pieceList[(pieceShah | (8 - g_toMove)) << 4], otherColor)) {
            UnmakeMove(move);
            return false;
        }
    } else {
        var kingPos = g_pieceList[(pieceShah | (8 - g_toMove)) << 4];
    ...
    if (flags < moveflagPromotion) {
        var theirKingPos = g_pieceList[(pieceShah | g_toMove) << 4];
        ...
        g_inCheck = IsSquareAttackable(g_pieceList[(pieceShah | g_toMove) << 4], 8 - g_toMove);
    }
...
function UnmakeMove(move){
    ...
    if (flags & moveflagPromotion) {
        piece = (g_board[to] & (~0x7)) | pieceSarbaz;
        g_board[from] = piece;
...
function ExposesCheck(from, kingPos){
    ...
    // If a rook can't reach it, nobody can!
    if ((g_vectorDelta[index].pieceMask[0] & (1 << (pieceRokh))) != 0) {
...
function GenerateAllMoves(moveStack) {
	...
	from = g_pieceList[pieceIdx++];
	while (from != 0) {
		to = from + 30; if (g_board[to] == 0) moveStack[moveStack.length] = GenerateMove(from, to);
		to = from + 34; if (g_board[to] == 0) moveStack[moveStack.length] = GenerateMove(from, to);
		to = from - 30; if (g_board[to] == 0) moveStack[moveStack.length] = GenerateMove(from, to);
		to = from - 34; if (g_board[to] == 0) moveStack[moveStack.length] = GenerateMove(from, to);
		from = g_pieceList[pieceIdx++];
	}
...
function GenerateCaptureMoves(moveStack) {
	...
	from = g_pieceList[pieceIdx++];
	while (from != 0) {
		to = from + 30; if (g_board[to] & enemy) moveStack[moveStack.length] = GenerateMove(from, to);
		to = from + 34; if (g_board[to] & enemy) moveStack[moveStack.length] = GenerateMove(from, to);
		to = from - 30; if (g_board[to] & enemy) moveStack[moveStack.length] = GenerateMove(from, to);
		to = from - 34; if (g_board[to] & enemy) moveStack[moveStack.length] = GenerateMove(from, to);
		from = g_pieceList[pieceIdx++];
	}
...
var g_seeValues = [0, 1, 3, 2, 5, 2, 900, 0,
                   0, 1, 3, 2, 5, 2, 900, 0];
...
function See(move) {
    ...
    if (((g_board[to + inc + 1] & 0xF) == (pieceSarbaz | them)) ||
        ((g_board[to + inc - 1] & 0xF) == (pieceSarbaz | them))) {
        return false;
    }
    ...
    if (((g_board[to - inc + 1] & 0xF) == (pieceSarbaz | us)) ||
        ((g_board[to - inc - 1] & 0xF) == (pieceSarbaz | us))) {
        g_board[from] = fromPiece;
        return true;
    }
